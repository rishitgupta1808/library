...
const checkIsDaysOfChargesUnderMinDays = (
  min_day: number,
  min_charges: number,
  ...
  if (min_day && min_charges) {
    //can be null as it is nullable true in entity
...
export const calculatePriceToBePaid = (
  ...
  try {
    ...
    if (!current_date)
      return {
        price: checkIsDaysOfChargesUnderMinDays(
          min_day,
          min_charges,
          days_to_return,
          price,
    ...
    if (dateBookToReturn > currentDate)
      return {
        price: checkIsDaysOfChargesUnderMinDays(
          min_day,
          min_charges,
          diffDateByDays(currentDate, lent_date),
          price,
          charges_in_min_day,
        ),
        isLate: false,
